...
public class RegEx {
  ...
  static final int NOTHING = 0xD07125;
  
  //automate : int[][] : lignes -> état, colonnes les caractères en ASCII
  ...
  public RegExTree parse() throws Exception {
    ...
    for (int i=0;i<regEx.length();i++) {
    	if((charToRoot(regEx.charAt(i)) == NOTHING) && (charToRoot(regEx.charAt(i+1)) == DOT) ){
    		result.add(new RegExTree((char)'.', new ArrayList<RegExTree>()));
    		i++;
    	}
    	else {
